...
INPBASE=hadmultkineimgs_127x94_me1Lmc
INPNAME=${INPBASE}.hdf5
NMAX=`ls $REMDIR/${INPBASE}_*.hdf5 | wc -l`
NMAX=$((NMAX - 1))
...
do
  filenum=`echo $i | perl -ne 'printf "%03d",$_;'`
  echo $filenum
  cp $REMDIR/${INPBASE}_${filenum}.hdf5 /data/minerva/DNN_hdf5/unrenamed/${INPBASE}_${filenum}.hdf5
  python rename_dataset.py --input $REMDIR/${INPBASE}_${filenum}.hdf5 \
...
INPBASE=hadmultkineimgs_127x94_me1Omc
INPNAME=${INPBASE}.hdf5
NMAX=`ls $REMDIR/${INPBASE}_*.hdf5 | wc -l`
NMAX=$((NMAX - 1))
...
INPBASE=hadmultkineimgs_127x94_me1Pmc
INPNAME=${INPBASE}.hdf5
NMAX=`ls $REMDIR/${INPBASE}_*.hdf5 | wc -l`
NMAX=$((NMAX - 1))
