...
def get_args_merge():
    ...
    parser.add_argument(
        "--n_events", action="store", dest="nevents",
        help="if supplied, add n_events from each file"
    )
...
def get_args_shuffle():

    """parse arguments for shuffle"""

    parser = argparse.ArgumentParser(
        description="HDF5 MANIPULATOR (merge)",
        usage="./shuffle.py <options>"
        )
    ...
    required.add_argument(
        "--input", action="store", dest="input",
        metavar="[path/to/filename]", required=True,
        help="path to input hdf5 file to shuffle"
    )
