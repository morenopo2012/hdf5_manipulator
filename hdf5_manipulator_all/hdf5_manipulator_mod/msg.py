...
def list_dataset(data, indent=1):
    ...
    for key in data:
        ...
        for member in data[key]:
           ...
           % {"key": (member+':').ljust(adjust),
               "size": '-> ' + str(data[key][member].shape),
               "type": ('[' + str(data[key][member].dtype) + ']').ljust(9),
              })
#    '''
#    print "data.keys()  ", data.keys()
#    adjust = len(max(data.keys(), key=len)) + 1  # length of left text column
...
def print_sample(data, key, member, start, end):
    print(data[key][member][start:end])
